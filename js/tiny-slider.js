    ...
    var R = function (t) {
        ...
        if (
            ...
            return {
                ...
                destroy: function () {
                    if (
                        ((ne.disabled = !0), ne.ownerNode && ne.ownerNode.remove(), H(l, { resize: $n }), qt && H(n, Re), Ue && H(Ue, Oe), an && H(an, De), H(gt, He), H(gt, ke), bn && H(bn, { click: Gi }), _t && clearInterval(pn), rt && Z)
                    ) {
                        var e = {};
                        (e[Z] = Ii), H(gt, e);
                    }
        ...
        function _n() {
            if (
                ...
                bn
                    ? C(bn, { "data-action": e })
                    : t.autoplayButtonOutput && (mt.insertAdjacentHTML(Pn(t.autoplayPosition), '<button data-action="' + e + '">' + wn[0] + e + wn[1] + $t[0] + "</button>"), (bn = mt.querySelector("[data-action]"))),
                    bn && D(bn, { click: Gi }),
            ...
            if (!Be)
                if ("prev" === t) zi(e, -1);
                    ...
                    var n = kn(),
                        i = 0;
                    if (
                        ("first" === t ? (i = -n) : "last" === t ? (i = rt ? wt - zt - n : wt - 1 - n) : ("number" != typeof t && (t = parseInt(t)), isNaN(t) || (e || (t = Math.max(0, Math.min(wt - 1, t))), (i = t - n))),
                        !rt && i && Math.abs(i) < zt)
                    ) {
                        var a = 0 < i ? 1 : -1;
